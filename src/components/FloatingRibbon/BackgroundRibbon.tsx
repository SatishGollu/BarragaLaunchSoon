...
const BackgroundRibbon: React.FC<BackgroundRibbonProps> = ({
  opacity = 0.15,
  speed = 0.005,
  color = "#ffffff",
}) => {
  ...
  useEffect(() => {
    ...
    // Create ribbon mesh with modified shader material
      ...
      new THREE.ShaderMaterial({
        ...
        vertexShader: `
          ...
          float xmb_noise2(vec3 x) {
            return cos(x.z * 4.0) * cos(x.z + time / 20.0 + x.x);
          }
          ...
          void main() {
            ...
            v.z -= noise(v3 * 5.0) / 20.0;
            v.y -= noise(v3 * 5.0) / 20.0 + cos(v.x * 1.5 - time / 3.0) / 8.0 - 0.2;
    ...
    const resize = () => {
      ...
      ribbon.scale.set(camera.aspect * 2, 1.2, 1);
    };
  ...
  return <div className="background-ribbon-container" ref={containerRef}></div>;
};
